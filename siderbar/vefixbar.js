;(function(vefixbar) {
    ...
    near:function(ele,option,def){    
        ...
        switch(option){
                case 'left' :               
                      that.bar.css({'left':rect_ele.left-thebarrect.width-def});
                break;
                case 'right':
                    that.bar.css({'left':rect_ele.right+def});
                break;
                case 'top':
                break;
                case 'bottom':
    ...
    follow:function(step,delay){
        ...
        $(window).scroll(function(){
            var _rect = __getRect(nele);
                 var docST = document.documentElement.scrollTop||document.body.scrollTop;
                 var to = docST +step;
                 $(nele).stop().animate({'top':+to,'opacity':1},300);
         });
            return this;
    },
